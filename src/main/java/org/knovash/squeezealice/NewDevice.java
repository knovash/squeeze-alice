...
//        JsonUtils.listToJsonFile(Home.devices, "home_devices.json"); // сделано в create
        return "CREATED " + id + " HOME: " + Home.devices.size();
    }
    ...
    public static String edit(HashMap<String, String> parameters) {
        ...
        String room = parameters.get("room");
        Home.devices.get(id).name=speaker_name_alice;
        Home.devices.get(id).customData.lmsName=speaker_name_lms;
        Home.devices.get(id).room=room;
        JsonUtils.listToJsonFile(Home.devices, "home_devices.json");
        return "EDITED";
    }

    public static String remove(HashMap<String, String> parameters) {
        String idd = parameters.get("id");
        Device device = Home.devices.stream().filter(d -> d.id.equals(idd)).findFirst().get();
        Home.devices.remove(device);
        JsonUtils.listToJsonFile(Home.devices, "home_devices.json");
        return "REMOVED " + idd + " HOME: " + Home.devices.size();
    }
...
//        myDevice.capabilities.add(mute);
        ...
        log.info("HOME SIZE BEFORE ADD: " + Home.devices.size());
        Integer id = myDevice.addToHome(myDevice);
        log.info("NEW DEVICE ID: " + id);
        log.info("HOME SIZE AFTER ADD: " + Home.devices.size());
        log.info("HOME DEVICES: " + Home.devices.stream().map(d -> d.customData.lmsName + " id=" + d.id).collect(Collectors.toList()));
        JsonUtils.listToJsonFile(Home.devices, "home_devices.json");
        return id;
    }
}