...
//        BODY: {"devices":[{"id":"0"}]}
            Integer device_id = Integer.valueOf(JsonUtils.jsonGetValue(body, "id"));
            log.info("QUERY FOR STATE DEVICE ID: " + device_id +
                    " NAME: " + Home.devices.get(device_id).name +
                    " LMS NAME: " + Home.devices.get(device_id).customData.lmsName);

            Payload payload = JsonUtils.jsonToPojo(body, Payload.class);
            log.info("QUERY PAYLOAD: " + payload);
    ...
    public static String updateDevice(Integer device_id) {
        ...
        Player player = Server.playerByName(Home.devices.get(device_id).customData.lmsName);
        Integer volume = 0;
        Boolean power = false;
        if (player != null) {
